"""
CDDAOYUE (次元乐) crawler for fetching detailed book information.

CDDAOYUE is an anime-focused novel platform with light novels.
"""
...
class CDDAOYUECrawler(BaseSourceCrawler):
    """
    Crawler for CDDAOYUE (次元乐) book detail pages.

    Extracts comprehensive book information including:
    - Basic metadata (title, author, description)
    - Category and tags (anime, light novel focused)
    ...
    def parse_book_detail(self, html: str, book_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not book_detail.get('title') or book_detail['title'] == f"Book {book_id}":
                logger.warning(f"Could not extract title from CDDAOYUE book {book_id}")
                return None

            return book_detail

        except Exception as e:
            logger.error(f"Error parsing CDDAOYUE book {book_id}: {e}")
            return None
