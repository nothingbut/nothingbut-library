"""
Youdubook (柚豆书) crawler for fetching detailed book information.

Youdubook is a mid-sized novel platform with relatively simple structure.
"""
...
class YoudubookCrawler(BaseSourceCrawler):
    """
    Crawler for Youdubook (柚豆书) book detail pages.
    ...
    def parse_book_detail(self, html: str, book_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not book_detail.get('title') or book_detail['title'] == f"Book {book_id}":
                logger.warning(f"Could not extract title from Youdubook book {book_id}")
                return None

            return book_detail

        except Exception as e:
            logger.error(f"Error parsing Youdubook book {book_id}: {e}")
            return None
    ...
    def _extract_rating(self, text: str) -> float:
        ...
        Args:
            text: Text containing rating (e.g., "8.5分", "8.5")
