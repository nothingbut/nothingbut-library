"""
Qidian (起点中文网) crawler for fetching detailed book information.

Qidian book pages typically have structure like:
- Title in .book-info h1 or .book-name
- Author in .book-info .writer
- Description in .book-intro
- Category in .book-info .type
- Cover in .book-img img
- Word count and other stats in .book-info .tag
"""
...
class QidianCrawler(BaseSourceCrawler):
    """
    Crawler for Qidian (起点中文网) book detail pages.
    ...
    def parse_book_detail(self, html: str, book_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Sub-category (second category level)
            book_detail['sub_category'] = (
                self._extract_text_multi(soup, [
                    '.book-info .type a:nth-child(2)',
            ...
            if not book_detail.get('title') or book_detail['title'] == f"Book {book_id}":
                logger.warning(f"Could not extract title from Qidian book {book_id}")
                return None

            return book_detail

        except Exception as e:
            logger.error(f"Error parsing Qidian book {book_id}: {e}")
            return None
